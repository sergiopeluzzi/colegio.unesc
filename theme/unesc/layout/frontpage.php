...
/**
 * Frontpage layout for the unesc theme.
 *
 * @package   theme_unesc
 */
...
$themesettings = new \theme_unesc\util\theme_settings();
...
if (isloggedin()) {
    ...
    theme_unesc_extend_flat_navigation($PAGE->flatnav);

    $templatecontext['flatnavigation'] = $PAGE->flatnav;

    $templatecontext = array_merge($templatecontext, $themesettings->footer_items(), $themesettings->slideshow());

    echo $OUTPUT->render_from_template('theme_unesc/frontpage', $templatecontext);
} else {
    $sliderfrontpage = false;
    if ((theme_unesc_get_setting('sliderenabled', true) == true) && (theme_unesc_get_setting('sliderfrontpage', true) == true)) {
        $sliderfrontpage = true;
        $extraclasses[] = 'slideshow';
    }

    $numbersfrontpage = false;
    if (theme_unesc_get_setting('numbersfrontpage', true) == true) {
        $numbersfrontpage = true;
    }

    $sponsorsfrontpage = false;
    if (theme_unesc_get_setting('sponsorsfrontpage', true) == true) {
        $sponsorsfrontpage = true;
    }

    $clientsfrontpage = false;
    if (theme_unesc_get_setting('clientsfrontpage', true) == true) {
        $clientsfrontpage = true;
    }

    $bannerheading = '';
    if (!empty($PAGE->theme->settings->bannerheading)) {
        $bannerheading = theme_unesc_get_setting('bannerheading', true);
    }

    $bannercontent = '';
    if (!empty($PAGE->theme->settings->bannercontent)) {
        $bannercontent = theme_unesc_get_setting('bannercontent', true);
    }

    $shoulddisplaymarketing = false;
    if (theme_unesc_get_setting('displaymarketingbox', true) == true) {
        $shoulddisplaymarketing = true;
    }

    $disablefrontpageloginbox = false;
    if (theme_unesc_get_setting('disablefrontpageloginbox', true) == true) {
    ...
    echo $OUTPUT->render_from_template('theme_unesc/frontpage_guest', $templatecontext);
}
