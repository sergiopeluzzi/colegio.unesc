 ...
 * @return string
 */
function theme_unesc_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_unesc_set_headerimg($theme);

    $scss .= theme_unesc_set_topfooterimg($theme);

    $scss .= theme_unesc_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_unesc_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_unesc_set_topfooterimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_unesc_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.unesc-login #page-wrapper #page {background-image: url('$loginbgimg');}";
 ...
 * @return string
 */
function theme_unesc_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_unesc', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_unesc and not theme_boost (see the line above).
    ...
    // unesc scss.
    $unescvariables = file_get_contents($CFG->dirroot . '/theme/unesc/scss/unesc/_variables.scss');
    $unesc = file_get_contents($CFG->dirroot . '/theme/unesc/scss/unesc.scss');

    // Combine them together.
    return $unescvariables . "\n" . $scss . "\n" . $unesc;
}
 ...
 * @return string
 */
function theme_unesc_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_unesc_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('unesc');
 ...
 * @return string
 */
function theme_unesc_get_setting($setting, $format = false) {
    $theme = theme_config::load('unesc');
...
/**
 * Extend the Unesc navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_unesc_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_unesc_rebuildcoursesections($flatnav);

    theme_unesc_delete_menuitems($flatnav);
}
 ...
 * @param flat_navigation $flatnav
 */
function theme_unesc_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_unesc_rebuildcoursesections(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_unesc'),
            'shorttext' => get_string('coursesections', 'theme_unesc'),
