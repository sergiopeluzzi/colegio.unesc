...
class block_cocoon_programs_edit_form extends block_edit_form {
    protected function specific_definition($mform) {
        ...
        // Title
        $mform->addElement('text', 'config_title', get_string('config_title', 'block_cocoon_tablets'));
        $mform->setDefault('config_title', 'Smarty Programs');
        $mform->setType('config_title', PARAM_RAW);

        // Subtitle
        $mform->addElement('text', 'config_subtitle', get_string('config_subtitle', 'block_cocoon_tablets'));
        $mform->setDefault('config_subtitle', 'Cum doctus civibus efficiantur in imperdiet deterruisset.');
        $mform->setType('config_subtitle', PARAM_RAW);

        // Image
        $mform->addElement('filemanager', 'config_image', get_string('config_image', 'block_cocoon_tablets'), null,
                array('subdirs' => 0, 'maxbytes' => $maxbytes, 'areamaxbytes' => 10485760, 'maxfiles' => 1,
                'accepted_types' => array('.png', '.jpg', '.gif') ));

        $slidesrange = range(0, 4);
        $mform->addElement('select', 'config_slidesnumber', get_string('slides_number', 'block_cocoon_programs'), $slidesrange);
        $mform->setDefault('config_slidesnumber', $data->slidesnumber);

        for($i = 1; $i <= $data->slidesnumber; $i++) {
            $mform->addElement('header', 'config_header' . $i , 'Program ' . $i);

            $mform->addElement('text', 'config_slide_title' . $i, get_string('config_slide_title', 'block_cocoon_programs', $i));
            $mform->setDefault('config_slide_title' .$i , 'Toddler (1,5 – 2 years)');
            $mform->setType('config_slide_title' . $i, PARAM_TEXT);

            $mform->addElement('text', 'config_slide_subtitle' . $i, get_string('config_slide_subtitle', 'block_cocoon_programs', $i));
            ...
            $f = $mform->addElement('filemanager', 'config_file_slide' . $i, get_string('file_slide', 'block_cocoon_programs', $i), null, $filemanageroptions);
        }
