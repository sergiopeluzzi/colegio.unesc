...
class block_cocoon_parallax_apps extends block_base {
    ...
    function specialization() {
        if (isset($this->config->title)) {
            ...
            $this->title = get_string('newcustomsliderblock', 'block_cocoon_parallax_apps');
        }
    }
    ...
    function get_content() {
        ...
        for ($i = 1; $i <= 2; $i++) {
            ...
            $text = '			<section class="home1-divider2 parallax" data-stellar-background-ratio="0.3" style="background-image:url('.$img_item[0].');">
		<div class="container">
			<div class="row">
				<div class="col-md-7 col-lg-7">
					<div class="app_grid">
						<h1 class="mt0">'.format_text($data->title, FORMAT_HTML, array('filter' => true)).'</h1>
						<p>'.format_text($data->subtitle, FORMAT_HTML, array('filter' => true)).'</p>
						<a class="ccn-app-grid-btn" href="'.format_text($data->app_store_btn_link, FORMAT_HTML, array('filter' => true)).'"><button class="apple_btn btn-transparent">
							<span class="icon">
								<span class="flaticon-apple"></span>
							</span>
							<span class="title">'.format_text($data->app_store_btn_title, FORMAT_HTML, array('filter' => true)).'</span>
							<span class="subtitle">'.format_text($data->app_store_btn_subtitle, FORMAT_HTML, array('filter' => true)).'</span>
						</button>
            </a>
            <a class="ccn-app-grid-btn" href="'.format_text($data->play_store_btn_link, FORMAT_HTML, array('filter' => true)).'">
						<button class="play_store_btn btn-transparent">
							<span class="icon">
								<span class="flaticon-google-play"></span>
							</span>
							<span class="title">'.format_text($data->play_store_btn_title, FORMAT_HTML, array('filter' => true)).'</span>
							<span class="subtitle">'.format_text($data->play_store_btn_subtitle, FORMAT_HTML, array('filter' => true)).'</span>
						</button>
            </a>
					</div>
				</div>
				<div class="col-md-5 col-lg-5">
					<div class="phone_img">
						<img class="img-fluid" src="'.$img_item[1].'" alt="">
					</div>
				</div>
			</div>
		</div>
