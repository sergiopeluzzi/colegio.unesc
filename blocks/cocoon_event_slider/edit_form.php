...
class block_cocoon_event_slider_edit_form extends block_edit_form {
    protected function specific_definition($mform) {
        ...
        $mform->addElement('select', 'config_slidesnumber', get_string('slides_number', 'block_cocoon_event_slider'), $slidesrange);
        $mform->setDefault('config_slidesnumber', $data->slidesnumber);

        for($i = 1; $i <= $data->slidesnumber; $i++) {
            $mform->addElement('header', 'config_header' . $i , 'Slide ' . $i);

            $mform->addElement('text', 'config_slide_title' . $i, get_string('config_slide_title', 'block_cocoon_event_slider', $i));
            $mform->setDefault('config_slide_title' .$i , 'An Overworked Newspaper Editor');
            $mform->setType('config_slide_title' . $i, PARAM_TEXT);

            $mform->addElement('date_selector', 'config_slide_date' . $i, get_string('config_slide_date', 'block_cocoon_event_slider', $i));

            $mform->addElement('text', 'config_slide_location' . $i, get_string('config_slide_location', 'block_cocoon_event_slider', $i));
            $mform->setDefault('config_slide_location' .$i , 'Vancouver, Canada');
            $mform->setType('config_slide_location' . $i, PARAM_TEXT);

            $mform->addElement('text', 'config_slide_time' . $i, get_string('config_slide_time', 'block_cocoon_event_slider', $i));
            ...
            $f = $mform->addElement('filemanager', 'config_file_slide' . $i, get_string('file_slide', 'block_cocoon_event_slider', $i), null, $filemanageroptions);
        }
