...
class block_cocoon_login extends block_base {
    function init() {
        $this->title = get_string('pluginname', 'block_cocoon_login');
    }
    ...
    function get_content () {
        ...
        if (!isloggedin() or isguestuser()) {   // Show the block
            ...
            $this->content->text .= '<input type="text" name="username" placeholder="'.get_string('username', 'theme_edumy').'" id="login_username" ';
            $this->content->text .= ' class="form-control" value="'.s($username).'" autocomplete="username"/></div>';

            $this->content->text .= '<div class="form-group">';

            $this->content->text .= '<input type="password" name="password" id="login_password" placeholder="'.get_string('password', 'theme_edumy').'" ';
            ...
            $this->content->text .= '<button type="submit" class="btn btn-log btn-block btn-thm2">'.get_string('login').'</button>';
