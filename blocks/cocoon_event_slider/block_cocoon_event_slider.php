...
class block_cocoon_event_slider extends block_base {
    ...
    function specialization() {
        if (isset($this->config->title)) {
            ...
            $this->title = get_string('newcustomsliderblock', 'block_cocoon_event_slider');
        }
    }
			...
			<div class="row">
				<div class="col-lg-6 offset-lg-3">
					<div class="main-title text-center">
						<h3 class="mt0">'.format_text($ccn_title, FORMAT_HTML, array('filter' => true)).'</h3>
						<p>'.format_text($ccn_subtitle, FORMAT_HTML, array('filter' => true)).'</p>
					</div>
				</div>
			</div>
			<div class="row">
                ...
                if (!empty($data->$sliderimage)) {
                  ...
                  if (!empty($data->$sliderimage)) {
								$text .='<div class="bph2_date_meta">
										      <span class="year">'.userdate($data->$slide_date, '%d', 0).' <br> '.userdate($data->$slide_date, '%B', 0).'</span>
									       </div>';
                }
								$text .='
                </div>
